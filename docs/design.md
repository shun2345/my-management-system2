...
第一フェーズでは**ダークモードは未実装**とする。YMYLメディアの読み手は明るい背景での閲覧が圧倒的多数で、優先度が低い。実装するとしても、栞の朱が暗背景で映えにくいため別パレットの設計が必要。当面は `prefers-color-scheme` を無視する。
...
記事本文の 720px は、1行あたり40文字前後に収めるための値。長すぎる行は読み戻りが起き、滞在時間と理解度を下げる。
...
- 形状: 矩形、`bg-brand-vermilion text-white`
- サイズ: 横10px×縦4px、フォント12px、weight 700
